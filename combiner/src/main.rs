...
impl FLoatingImage{
    ...
    fn set_data(&mut self, data:Vec<u8>)-> Result<(),ImageDataErrors>{
        ...
        println!("datalen= {:?}, capacity= {:?}",data.len(),self.data.capacity());
        self.data = data;
        Ok(())
...
fn stadardise_size(image_1:DynamicImage,image_2:DynamicImage) ->(DynamicImage,DynamicImage){
    let (width,heigth) = get_smallest_dimension(image_1.dimensions(), image_2.dimensions());
    println!("width: {},height: {}\n",width,heigth);
    if image_2.dimensions() == (width,heigth){
        (image_1.resize_exact(width, heigth, Triangle),image_2)
    } else{
        (image_1,image_2.resize_exact(width, heigth, Triangle))
    }
}
